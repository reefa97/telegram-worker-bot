...
-- We need to drop existing SELECT policies and re-create them with "AND deleted_at IS NULL"
...
CREATE POLICY "Admins can view accessible workers" ON workers 
    FOR SELECT TO authenticated 
    USING (user_can_access_worker(auth.uid(), id) AND deleted_at IS NULL);

-- object_tasks (Need to check existing policy name or create new)
...
-- 3. Create Secure Functions for Trash Can Management
-- These functions use SECURITY DEFINER to bypass RLS and access deleted items

CREATE OR REPLACE FUNCTION get_trashed_items(item_type TEXT)
RETURNS TABLE (
    id UUID,
    name TEXT,
    info TEXT, -- Additional info (e.g. object name for tasks)
