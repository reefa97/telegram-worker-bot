...
async function applyMigration(filePath) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Fallback: Try the /rest/v1/rpc/exec endpoint if pg/migrations failed
            // Note: This requires a function named 'exec' or similar to exist.
            // But let's verify if the error is essentially "already exists" or something else.
            return false;
        }
    } catch (error) {
