...
serve(async (req) => {
    ...
    try {
        ...
        if (caller?.role !== "super_admin") {
            return new Response(JSON.stringify({ error: "Only super_admin can delete sub-admins/managers" }),
        ...
        // Verify target is actually sub_admin/manager (not super_admin or client)
        ...
        if (!["sub_admin", "manager"].includes(target.role)) {
            return new Response(JSON.stringify({ error: "This endpoint only deletes sub_admin/manager" }),
                { status: 400, headers: { ...cors, "Content-Type": "application/json" } });
        }
