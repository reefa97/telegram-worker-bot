-- RLS Hardening for Quality Control Module v2
-- This script ensures consistent and non-recursive policies for all QC tables.

-- Ensure admin_users has a simple and fast SELECT policy for EXITS/JOIN checks
...
CREATE POLICY "Admins can view all admin profiles" ON admin_users
  ...
  USING (EXISTS (SELECT 1 FROM admin_users WHERE id = auth.uid() AND role IN ('super_admin', 'sub_admin')));
...
DROP POLICY IF EXISTS "Anyone can view check items" ON quality_check_items;
...
DROP POLICY IF EXISTS "Anyone can view schedules" ON quality_check_schedules;
...
-- 4. worker_points_log
DROP POLICY IF EXISTS "Anyone can view points log" ON worker_points_log;
